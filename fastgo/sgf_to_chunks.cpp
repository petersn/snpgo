...
//#define SELECT_JUST_ONE_MOVE
//#define SCORING
//#define RANK_THRESHOLD 4
//#define RANK_THRESHOLD -100
//#define RANK_THRESHOLD -100
#define RANK_THRESHOLD 9
//#define SELF_PLAY

class RoundRobinWriter {
	// Here we use lists instead of vectors because filtering_ostream deletes copy.
	list<ofstream> files;
	list<filtering_ostream> streams;
	list<filtering_ostream>::iterator here;

	int count;
public:

	int index = 0;
	RoundRobinWriter(string base_path, int count) : count(count) {
		for (int i = 0; i < count; i++) {
			files.emplace_back(base_path + "_" + std::to_string(i), ios_base::out | ios_base::binary);
...
//					cerr << "Contains move AW/BW/AE, which is currently not supported: " << path << endl;
					return false;
				} else if (property_name == "HA") {
//					cerr << "Contains unsupported handicap encoded in move: " << path << endl;
					return false;
				}
#ifdef SELF_PLAY
...
//			if (piece == (int)perspective_player) {
				...
				FEATURE(FEAT_P1_LIBERTY4)           = liberties == 4;
				FEATURE(FEAT_P1_LIBERTY5)           = liberties == 5;
				FEATURE(FEAT_P1_LIBERTY6)           = liberties == 6;
				FEATURE(FEAT_P1_LIBERTY7)           = liberties == 7;
				FEATURE(FEAT_P1_LIBERTY8PLUS)       = liberties >= 8;
//			} else {
//				FEATURE(FEAT_P2_LIBERTY1)           = liberties == 1;
//				FEATURE(FEAT_P2_LIBERTY2)           = liberties == 2;
//				FEATURE(FEAT_P2_LIBERTY3)           = liberties == 3;
//				FEATURE(FEAT_P2_LIBERTY4)           = liberties == 4;
//				FEATURE(FEAT_P2_LIBERTY5)           = liberties == 5;
//				FEATURE(FEAT_P2_LIBERTY6)           = liberties == 6;
//				FEATURE(FEAT_P2_LIBERTY7)           = liberties == 7;
//				FEATURE(FEAT_P2_LIBERTY8PLUS)       = liberties >= 8;
//			}
...
void write_all_samples(
	RoundRobinWriter& features_writer,
	RoundRobinWriter& targets_writer,
	RoundRobinWriter& winners_writer,
	RoundRobinWriter& territory_writer,
	string path
) {
#else
void write_all_samples(RoundRobinWriter& features_writer, RoundRobinWriter& targets_writer, RoundRobinWriter& winners_writer, string path) {
#endif
...
//		cerr << "Black too low rank: " << game.white_rank << " " << game.black_rank << endl;
	...
	for (int move_index = 0; move_index < game.moves.size(); move_index++) {
		...
		if (do_write_this_move)
			write_features(features_writer.get_filtering_ostream(), board, m.who_moved);
		...
		for (int which_history = 0; which_history < 2; which_history++) {
			...
			for (int i = 0; i < HISTORY_LENGTH; i++) {
				...
				if (do_write_this_move)
					features_writer.write(reinterpret_cast<const char*>(&entry[0]), BOARD_SIZE * BOARD_SIZE);
			}
		}
...
//		territory_writer.write(final_territory, final_territory.size());
		if (m.who_moved == Player::BLACK) {
			territory_writer.get_filtering_ostream() << final_territory_black;
		} else if (m.who_moved == Player::WHITE) {
			territory_writer.get_filtering_ostream() << final_territory_white;
		} else {
			assert(false);
		}
#endif
...
int main(int argc, char** argv) {
	if (argc != 8) {
		cerr << "Usage: sgf_to_chunks root_directory features_chunk.z targets_chunk.z winners_chunk.z start_index stop_index round_robin_count" << endl;
		cerr << endl;
	...
	int round_robin_count = stoi(argv[7]);
...
*/
	...
	RoundRobinWriter territory_writer(territory_chunk_path, round_robin_count);
#endif
...
//		cout << path << endl;
#ifdef SCORING
		write_all_samples(features_writer, targets_writer, winners_writer, territory_writer, path);
#else
